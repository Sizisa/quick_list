      ...
      <template #item="{ element, index }">
        <div>
          <el-form-item>
            ...
            <el-button link type="primary" size="small" @click="removeSearch(index)"
              >删除</el-button
            >
          </el-form-item>
        </div>
...
const save = function () {
  if (props.currentIndex != undefined && props.currentIndex === -1) {
    ...
    if (store.data[props.currentIndex].searchItems) {
      store.data[props.currentIndex].searchItems.splice(
        0,
        store.data[props.currentIndex].searchItems.length
      )

<template>
  <el-container>
    ...
    <el-main>
      <el-button type="primary" plain :icon="Plus" @click="openEditDialog(-1)">新增</el-button>
      <el-popconfirm
        title="该操作会读取剪切板中的配置，并将配置覆盖！"
      ...
      <el-table :data="tableData" stripe style="width: 100%">
        ...
        <el-table-column label="操作" width="180" fixed="right">
          <template #default="scope">
            ...
            <el-button link type="primary" size="small" @click="openEditDialog(scope.$index)"
              >编辑</el-button
            >
  ...
  <el-dialog v-model="showSearchDialog" title="搜索" width="500" destroy-on-close>
    <el-form-item v-for="(value, key) in searchLabel" :label="key" :key="key">
...
const openPage = function (scope: any) {
  ...
  for (let key in searchLabel) {
    if (Object.prototype.hasOwnProperty.call(searchLabel, key)) {
...
const search = function () {
  ...
  window.open(
    window.location.href + 'list/' + currentIndex.value + '?' + query.substring(1, query.length),
    '_blank'
  )
}
